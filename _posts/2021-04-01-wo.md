...
\shoveleft \implies O, D, A \text{ collinear }\implies \angle{DAC}=\angle{OAC}=\bbox[5px, border: 1px solid black]{60^{\circ}}
\end{multline}
$$
---
...
$\implies \angle{DAB}=75^{\circ}-45^{\circ}=\bbox[5px, border: 1px solid black]{30^{\circ}}$
...
But $x \ge 0 \implies x=\bbox[5px, border: 1px solid black]{\dfrac{1+\sqrt{5}}{2}}$
