...
$\alpha \in (0， 40^{\circ}) \implies sin(2\alpha+60^{\circ}) \gt sin(40^{\circ}) \gt sin(20^{\circ})cos(\alpha+80^{\circ}) \implies f '(\alpha) \gt 0 \implies \angle{A}=\bbox[5px, border: 1px solid black]{20^{\circ}}$为唯一解。
...
$\alpha \in (0， 40^{\circ}) \implies sin(2\alpha+60^{\circ}) \gt sin(40^{\circ}) \gt sin(20^{\circ})cos(\alpha+80^{\circ}) \implies f '(\alpha) \gt 0 \implies \angle{A}=\bbox[5px, border: 1px solid black]{20^{\circ}}$ is the only solution.
...
从而只能 $CD=BE \implies \angle{C}=80^{\circ} \implies \angle{A}=\bbox[5px, border: 1px solid black]{20^{\circ}}$.
...
Then we know $CD=BE \implies \angle{C}=80^{\circ} \implies \angle{A}=\bbox[5px, border: 1px solid black]{20^{\circ}}$.
...
5. Calculate all distinct permutations of $1,1,2,2,3,3,4,4$ that contains $4$ pairs of equal consecutive numbers. Take each of pair of them as one number, so this is permutation of 4 numbers: $4!=24$.
   ...
   According to PIE, the result is $2520-2520+1080-240+24=\bbox[5px, border: 1px solid black]{864}$.
