...
$k$-th degree **power sum** polynomial for variables $x_{1},x_{2},...x_n$ is:

$p_k(x_{1},x_{2},...,x_n)=x_{1}^k+x_{2}^k+...+x_n^k$, so we know
...
$k$-th degree **elementary symmetric** polynomial for $x_{1},x_{2},...x_n$ is:

$e_{0}(x_{1},x_{2},...,x_n)=e_{0}=1$

$e_{1}(x_{1},x_{2},...,x_n)=e_{1}=x_{1}+x_{2}+...+x_n$

$e_{2}(x_{1},x_{2},...,x_n)=e_{2}=x_{1}x_{2}+x_{2}x_{3}+...+x_{n-1}x_n$

$e_k(x_{1},x_{2},...,x_n)=e_k=\displaystyle \sum_{1 \le j_{1} \lt ... \lt j_k \le n}{x_{j_{1}}x_{j_{2}}...x_{j_k}}$

$e_r(x_{1},x_{2},...,x_n)=0 (r>n)$
...
$f(t)=t^n+a_{n-1}t^{n-1}+...+a_{2}t^2+a_{1}t+a_{0}$ has $n$ roots $x_{1},x_{2},...,x_n$

$f(t)=(t-x_{1})(t-x_{2})...(t-x_n)=t^n-e_{1}t^{n-1}+e_{2}t^{n-2}+...+(-1)^ne_n$

$f(x_{1})=0=x_{1}^n-e_{1}x_{1}^{n-1}+...+(-1)^ne_n$

$f(x_{2})=0=x_{2}^n-e_{1}x_{2}^{n-1}+...+(-1)^ne_n$

$...$

$f(x_n)=0=x_n^n-e_{1}x_n^{n-1}+...+(-1)^ne_n$

Sum above $n$ equations to get

$0=p_n-e_{1}p_{n-1}+...+n(-1)^ne_n$

**[Girard-Newton Identities](https://en.wikipedia.org/wiki/Newton%27s_identities)**:

$\boxed{ke_k=e_{k-1}p_{1}-e_{k-2}p_{2}+...+(-1)^{k-1}e_{0}p_k=\displaystyle \sum_{i=1}^{k}{(-1)^{i-1}e_{k-i}p_i} \qquad (n \ge k \ge 1)}$

Back to the problem:

$e_{0}=1, e_{1}=x+y+z=1, e_{2}=xy+yz+zx, e_{3}=xyz, e_r=0 \text{ } (r \gt 3)$
