...
We have $n \gt 2, a \gt b, k=\dfrac{a^n+b^n}{(ab)^{n-1}+1}, a,b,k,n \in \mathbb{N} \implies a^n-k=(ka^{n-1}-b)b^{n-1}$

​	(i) if $k \gt a^n$, then $a^n-k=(ka^{n-1}-b)b^{n-1} < 0 \implies a \gt b \gt ka^{n-1}\ge k \gt a^n \implies a \gt a^n$ this is impossible

​	(II) if $k < a^n$, then $kb^{n-1}-a=\dfrac{b^n-k}{a^{n-1}}<\dfrac{b^n}{a^{n-1}}<\dfrac{b^n}{b^{n-1}}=b \implies kb^{n-1} \lt a+b \implies kb^{n-1}-b < a \implies kb^{n-1}-b^{n-1} < a \implies (k-1)b^{n-1} < a$

$k > 1 \implies b^{n-1} \le (k-1)b^{n-1}<a \implies b^n-k=(kb^{n-1}-a)a^{n-1} \ge a^{n-1} > (b^{n-1})^{n-1} \ge b^n$ this is impossible
...
![image-20211214230431450](/assets/images/2021-12/image-20211214230431450.png)

**Solve:**

Let $F$ on $AC$ so that $\angle{FBC=20^{\circ}}$, easy to know that $BC=BF$.

$\angle{BCE}=\angle{BEC}=50^{\circ} \implies BE=BC, \angle{EBF}=60^{\circ} \implies \triangle{BEF}$ is equilateral triangle

$\implies BE=BC=BF=EF$

$\angle{BDC}=\angle{FBD}=40^{\circ} \implies BF=DF \implies EF=DF$

$\angle{EFD}=180^{\circ}-60^{\circ}-80^{\circ}=40^{\circ} \implies \angle{EDF}=70^{\circ}\implies \angle{EDB}=\boxed{30^{\circ}}$
